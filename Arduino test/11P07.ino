...
// Servo Static Variables------------------------
#include <Servo.h>
// Arduino pin assignment
#define PIN_SERVO 10

// configurable parameters
#define _DUTY_MIN 650 // servo full clock-wise position (0 degree)
#define _DUTY_NEU 1825 // servo neutral position (90 degree)
#define _DUTY_MAX 2350 // servo full counter-clockwise position (180 degree)
...
void loop() {
  ...
  if ((dist_raw == 0.0) || (dist_raw > _DIST_MAX)) {
      dist_filtered = _DIST_MAX;    // Set Higher Value
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw < _DIST_MIN) {
      dist_filtered = _DIST_MIN;    // Set Lower Value
      digitalWrite(PIN_LED, 1);       // LED OFF
  ...
  dist_ema = dist_ema * (1-_EMA_ALPHA);
  dist_ema += dist_filtered * _EMA_ALPHA;
  ...
  duty_target = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);

  duty_curr = duty_target; //  servo_speedControl();
...
void servo_speedControl() { //Needs duty_target, Modifies duty_curr
